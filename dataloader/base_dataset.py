...
class BaseDataSet(Dataset):
    def __init__(self, root, split, mean, std, base_size=None, augment=True, val=False,
                crop_size=321, scale=True, flip=True, rotate=False, blur=False, return_filename=True):
        self.root = root
        self.split = split
        self.mean = mean
        self.std = std
...
#             h, w = (longside, int(1.0 * longside * w / h + 0.5)) if h > w else (int(1.0 * longside * h / w + 0.5), longside)
    ...
    def __getitem__(self, index):
        image, label, image_filename = self._load_data(index)
        ...
        if self.return_filename : 
            return self.normalize(self.to_tensor(image)), label, image_filename
        else : 
